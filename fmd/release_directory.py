...
try:
    ...
    class _StubRepo:
        ...
        def clone_from(*args, **kwargs):
            raise RuntimeError("git is required for cloning operations")
...
class BenchDirectory:
    ...
    def list_sites(self) -> list[Path]:
        if self.sites.exists() and self.sites.is_dir():
            return [site for site in self.sites.iterdir() if site.is_dir() and (site / "site_config.json").is_file()]
        return []
    ...
    def clone_app(self, app: AppConfig, clone_path: Path, move_to_subdir: bool = True) -> Path:
        ...
        if not app.is_ref_commit:
            ...
            cloned_repo = git.Repo.clone_from(app.repo_url, clone_path_tmp, depth=depth, origin=app.remote_name)
