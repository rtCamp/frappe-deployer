...
*Requirements*: Python >=3.13,<3.14, Docker + Frappe Manager

* Development Setup

** Pre-commit Hooks

This project uses [[https://pre-commit.com/][pre-commit]] for automated code quality checks before commits.

*Install hooks* (one-time setup):

#+begin_src bash
# Install pre-commit (if using uv)
...
- *Secret scanning*: Gitleaks detects hardcoded credentials, tokens, API keys
- *Code quality*: Ruff linting and formatting
- *Safety checks*: Private key detection, large file blocking, merge conflict markers
- *File validation*: YAML/TOML syntax checking, trailing whitespace, end-of-file fixes

*Manual check* (runs on all files):
...
*CI enforcement*: All checks also run in GitHub Actions on pull requests.

*Bypass hooks* (emergencies only, CI will still block):

#+begin_src bash
git commit --no-verify  # ⚠️ Avoid this - CI will fail anyway
#+end_src

See [[file:.pre-commit-config.yaml][.pre-commit-config.yaml]] for hook configuration.
...
- [[file:docs/getting-started/quick-start.md][Quick Start]] — Deploy your first site in minutes
- [[file:docs/guides/configuration.md][Configuration Guide]] — Complete config reference
- [[file:docs/guides/deploy-modes.md][Deploy Modes]] — Pull vs ship strategies
- [[file:docs/guides/github-actions.md][GitHub Actions]] — CI/CD integration
- [[file:docs/commands/index.md][Command Reference]] — All commands with examples
- [[file:docs/reference/troubleshooting.md][Troubleshooting]] — Common issues and solutions
- [[file:example-config.toml][example-config.toml]] — Complete config schema with comments
